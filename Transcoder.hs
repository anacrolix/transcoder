...
runProgressServer :: Transcoder -> HostPreference -> Int -> IO ()
runProgressServer t host port = do
    debugM rootLoggerName $
        "progress server starting at " <> show host <> ":" <> show port
    Warp.runSettings
        ( setHost host $
            setPort port $
                setOnException Transcoder.onException defaultSettings
        )
        $ progressApp
...
newTranscoder :: (Show a) => a -> IO Transcoder
newTranscoder progressAppPort = do
...
respondPartial method ranges size respond get =
  ...
  where
    (status, headers, offset) =
        case Transcoder.first ranges of
            Nothing ->
                ( status200
